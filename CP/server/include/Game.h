...
private:
    ...
    int remainingShips1;
    int remainingShips2;
    std::string player1BoardStr;
    std::string player2BoardStr;
...
public:
    ...
    std::string getBoardState(Player* player, bool showShips = false) const;
...
private:
    ...
    bool checkSunk(int x, int y, const std::vector<std::vector<bool>>& ships, 
                  const std::vector<std::vector<int>>& board) const;
    void markSunkShip(int x, int y, std::vector<std::vector<int>>& board,
